"""
Garmin Workout Dashboard
Full Backend with Browser-persistent Strength Integration
"""
...
class GarminSync:
    ...
    def fetch_training_readiness_comprehensive(self, latest_strength_date: str = None) -> dict:
        if not self.client:
            if not self.login():
        ...
        try:
            ...
            runs_date = run_row["date"].split("T")[0] if run_row and run_row["date"] else ""
            strength_date = latest_strength_date.split("T")[0] if latest_strength_date else ""

            if runs_date and strength_date:
                if strength_date >= runs_date:
                    latest_act = {"date": strength_date, "type": "strength"}
                else:
        ...
        if latest_act:
            try:
                ...
                if latest_act.get("type") == "strength":
                    if days_ago <= 0:
                        recent_penalty = 12
                    ...
                    dist_km = (latest_act.get("distance") or 0) / 1000
                    if days_ago <= 0:
...
async def get_training_readiness(latest_strength_date: str = Query(None)):
    return garmin_sync.fetch_training_readiness_comprehensive(latest_strength_date)
